...
**Rule:** In every reconciliation predicate that asks "does this deposit still have unbooked charge work?", a charge in a terminal status (`excluded`, `rejected`) must count as settled — never as open work. "No gift link" alone is NOT "open": terminal charges never get a gift link by design (a failed payment attempt auto-lands `excluded/failed_charge`; a human dismissal lands `rejected`).
...
**How to apply:**
- The terminal set is `NOT IN ('excluded','rejected')` — keep it consistent with the graph endpoint and bundle anchors, which already filter both.
...
- Sibling of the "resolved has 4 forms" lesson: staged-payment resolution has many shapes, and charge-side terminality is one more "not work" shape that id-column checks miss.
