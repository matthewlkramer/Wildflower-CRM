...
The reconciliation feature routers are mounted in `routes/index.ts` WITHOUT a path prefix — each route must self-prefix its full path (e.g. `"/reconciliation/deposits"`). A route registered with only `"/workbench-foo"` silently lives at `/api/workbench-foo` and the client's `/api/reconciliation/...` 404s.
