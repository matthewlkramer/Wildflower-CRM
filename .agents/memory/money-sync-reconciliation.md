...
- [QuickBooks payment grouping](quickbooks-deposit-grouping.md) — group-approve mints ONE gift atomically (count-guard or 409); QB group state stored as `unit_groups`/`unit_group_members` rows (NOT pointer columns); group id IS unit_groups.id; null date is its own bucket (client/server lockstep); cross-date needs confirmMultiDate.
