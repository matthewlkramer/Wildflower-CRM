...
- Deposit-workbench gift-column anchors must cover EVERY evidence kind that can back col-3 actions (charge, QB-staged, AND manual gift-less bank_spine components) — a missing kind renders the misleading "no unlinked evidence" blanket-disable. Manual units link to an existing gift via the adopt-unit path (`addBankDepositComponent mode:"gift"`, amount-targeted ±0.005, 409 if ambiguous); mint/pledge/identify stay disabled with an accurate no-QB-record reason because gifts mint only from QB/Stripe evidence. Handlers that assume "non-charge ⇒ staged" need explicit kind guards.
