...
- [QB full re-pull (background, non-destructive)](quickbooks-nondestructive-repull.md) — ~4min job exceeds proxy timeout→fire-and-forget + GET resync-status poll (advisory lock is the real guard); backfill qb\_\* via fullResync+enrichAllStatuses NOT a wipe; set stays read-only QB facts, qb_raw excluded ([polled](quickbooks-resync-background.md)).
...
- [QB deposit memo wrong donor](quickbooks-deposit-memo-wrong-donor.md) — card payer label = QB memo, can name the WRONG donor; the payout's charges are authoritative; check screenshot image\_<ms>.png epoch vs match_confirmed_at before claiming "didn't work".
...
- **Resolved QBO Deposit lines remain sync-owned facts** — incremental QBO upserts refresh amount/date/payer/capture fields on resolved `staged_payments`; bank-spine recompute refreshes the corresponding QBO-sourced unit/component before calculating capacity. An exact same-payer whole-deposit re-split may carry the sole gift pointer to new siblings only when every current line is represented and bank/gift/component totals agree; otherwise leave the siblings for review. Never allow component unlink while its unit still has `gift_id`, or fill-only recompute can resurrect a half-detached relationship.
...
- Deposit-workbench cards represent evidence, not work prompts: unresolved
  ...
  three-dot actions. Keep record-specific QBO actions (detail, unlink, exclude,
  correction disposition) on the individual accounting card; never have a
  column menu silently target its first record.
...
- Deposit-workbench gift-column anchors must cover EVERY evidence kind that can back col-3 actions (charge, QB-staged, AND gift-less bank_spine components) — a missing kind renders the misleading "no unlinked evidence" blanket-disable. A staged anchor is only valid while the QB row is actionable (pending); a row booked elsewhere/excluded/derived-excluded (e.g. by a confirmed charge tie) 409s every staged action, so the row must fall back to a component/unit anchor. Unit-backed component anchors support all four gift actions: link adopts the exact unit (`addBankDepositComponent mode:"gift"` with paymentUnitId; amount ±0.005 only as fallback), create-gift/identify-donor mint via the unit-anchored mint route, and Donorbox lookup stamps the donation onto the unit before booking. Only the pledge path still requires QB-staged evidence. Handlers that assume "non-charge ⇒ staged" need explicit kind guards.

## Imports/backfills & prod verification

- [Coding-form import staging](coding-form-import-staging.md) — one-time xlsx→coding*form_rows staging + compare-don't-clobber reconciliation; cross-check derived live on read, apply writes only reviewer-approved attrs, idempotent via stored applied*\* ids.
