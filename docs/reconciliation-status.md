...
| Manual gift creation on a pledge   | Blocked at `POST /gifts-and-payments` (`manual_gift_on_pledge_blocked`, Task #788) — pledge payments are minted from payment evidence via reconciliation. QB-first gating is retired (ratified 2026-07-31; the QBO entry is the last step): all three evidence mint paths accept the pledge — QuickBooks staged rows (approve outcome `create_gift_from_opportunity`), Stripe charges (`POST /stripe-staged-charges/:id/create-gift` with `opportunityId`), and payment units (`POST /reconciliation/payment-units/:id/create-gift` with `opportunityId`); each locks the pledge, requires a live written pledge, and copies its remaining plan. Sole escape hatch: the explicit finance-gated `offBooksException` request flag (money that never hits QuickBooks); the flag is never persisted. The awaiting-settlement manual mint on `POST /opportunities-and-pledges/:id/mint-gift` was removed (2026-07-29; 409 `awaiting_settlement_mint_removed`) — the opportunity page now routes users to the reconciliation workbench, keeping only the finance-gated off-books mint. Minted gifts inherit scope from the pledge's remaining plan (`copyPledgeAllocationsToGift`, stamped via `gift_allocations.source_pledge_allocation_id`)                                                         |
...
Forward maintenance also handles a QBO Deposit that is edited after one of its
direct lines was reconciled: sync refreshes source-owned facts on resolved
`staged_payments`; bank-spine recompute refreshes the QBO-sourced payment unit
and component amounts before calculating remaining capacity; and an exact,
same-payer, whole-deposit re-split carries the sole existing gift pointer to new
sibling units when the gift and deposit totals agree. Less certain shapes remain
gift-less for review. A component whose unit already funds a gift cannot be
unlinked from its deposit until the gift tie is removed, preventing a later
fill-only recompute from resurrecting a half-detached relationship.
