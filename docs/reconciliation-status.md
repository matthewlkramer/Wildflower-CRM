...
| Gifts and pledge payments from evidence | Manual gift creation on a pledge remains blocked at `POST /gifts-and-payments` (`manual_gift_on_pledge_blocked`, Task #788): pledge payments are minted from received-payment evidence in reconciliation. QB-first gating is retired (ratified 2026-07-31; the QBO entry is the last step). QuickBooks staged rows, Stripe charges, payment units, and unresolved deposit remainders can all be applied to a live finalized pledge. The same searches include open opportunities; choosing one requires an explicit choice to close it as a one-time gift or convert it to a pledge and record the first payment. All paths lock the fundraising record and payment evidence, derive the donor, and copy the remaining allocation plan. Sole escape hatch: the finance-gated `offBooksException` flag for money that never hits QuickBooks; the flag is never persisted. The awaiting-settlement manual mint remains removed, and the opportunity page routes users to the reconciliation workbench. |
