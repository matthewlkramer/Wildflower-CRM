...
- **API contract gaps** — recent schema additions are not yet reflected in `lib/api-spec/openapi.yaml` or downstream codegen: `household_id` on opps + gifts, `historical_names` on funders + organizations, the new `private_wealth_manager` payment-intermediary-type value, the new `donor_xor` CHECK constraint, and request-level validation for the donor XOR rule. Add these when you rewrite the spec.
