  ...
  try {
    ...
    try {
      ...
      await test.query(`
        ...
        RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
          DELETE FROM payment_units WHERE stripe_charge_id = OLD.id;
          RETURN OLD;
        END;
        $$;
        DROP TRIGGER IF EXISTS cleanup_stripe_payment_unit
          ON stripe_staged_charges;
        CREATE TRIGGER cleanup_stripe_payment_unit
          BEFORE DELETE ON stripe_staged_charges
          FOR EACH ROW
          EXECUTE FUNCTION test_meta.cleanup_stripe_payment_unit();
      `);
    } finally {
      await test.end();
    }
