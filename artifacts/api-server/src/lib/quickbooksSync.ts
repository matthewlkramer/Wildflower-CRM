...
export function buildStagedLineUpsert(
  ...
  const set = {
    ...
    qbLocation: sql`coalesce(excluded.qb_location, ${stagedPayments.qbLocation})`,
    ...
    await tx.insert(giftAllocations).values({
      ...
      fundableProjectId:
        rule.targetIntendedUsage === "project"
          ...
          : null,
      // Goal-counting now lives on the allocation. Government reimbursements are
      // real money that doesn't advance the fundraising goal → flag false here.
