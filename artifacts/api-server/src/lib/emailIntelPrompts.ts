...
const DEFAULT_ACCURACY_PROMPTS: Record<EmailIntelSignalType, string> = {
  ...
  signature_update: [
    ...
    "Mark inaccurate when the signature belongs to someone else (a forwarded message, a different signer), when the parsed 'company' is mis-parsed prose, or when the parsed value is a meeting dial-in rather than a personal phone.",
  ].join("\n"),
  grant_opportunity: [
