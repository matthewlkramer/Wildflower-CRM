...
export function buildActionProposingCorePrompt(): string {
  return [
    ...
    "    – company: treat it as changed ONLY if it doesn't match the name of ANY current role entity in context. A current role entity may list prior/alias names on a following '(same entity also known as — …)' line; a signature company that matches one of those FORMER or OTHER names is the SAME employer under an old/alternate name — treat it as UNCHANGED and emit NO action (no create_org_with_per / create_funder_with_per, no title change for a mere rename).",
