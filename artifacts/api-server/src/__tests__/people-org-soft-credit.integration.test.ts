 ...
 * Lifetime giving / Last gift include an ORGANIZATION's gift only when the
 * person is a *current principal* of the donor organization. Primary-contact
 * and advisor relationships are relationship-management context, not donor
 * credit.
 *
 * Invariants asserted here:
 *  - Org-credit applies only to organization-donor gifts, so it never
 *    double-counts a person's own direct / household giving.
 *  - Primary-contact and advisor-only relationships do not create credit.
 *  - A gift matching principal plus other relationship signals counts once.
...
describe.skipIf(!HAS_DB)(
  ...
  () => {
    it("does not credit a primary-contact-only person", async () => {
      ...
      expect(card?.lastGiftDate).toBeNull();
      expect(card?.lastGiftAmount).toBeNull();
    });
    ...
    it("does not credit an advisor-only person", async () => {
    ...
    it("blends direct + household + org-credit into one number, last gift is the latest across all", async () => {
      ...
      const detail = await getPerson(person);
      expect(detail.lifetimeGiving).toBe("300.00");
      expect(detail.mostRecentGiftDate).toBe("2021-01-01");

      const card = await getTopPriorityPerson(person);
      expect(card?.lastGiftDate).toBe("2021-01-01");
      expect(card?.lastGiftAmount).toBe("200.00");
    });
