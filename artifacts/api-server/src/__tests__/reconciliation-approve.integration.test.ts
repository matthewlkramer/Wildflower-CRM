...
async function seedStaged(
  ...
  await db.insert(schema.stagedPayments).values({
    ...
    ...(opts.autoApplied !== undefined
      ? { autoApplied: opts.autoApplied }
      : {}),
    ...(opts.organizationId !== undefined
      ? { organizationId: opts.organizationId }
      : {}),
    ...(opts.matchStatus !== undefined
      ? { matchStatus: opts.matchStatus }
      : {}),
    ...(opts.groupLinkedGiftId != null ? { matchConfirmedAt: new Date() } : {}),
  });
  stagedIds.push(id);
  const ledgerGiftId =
...
describe.skipIf(!HAS_DB)(
  "Reconciliation approve — link existing gift (integration)",
  () => {
...
describe.skipIf(!HAS_DB)(
  "Reconciliation approve — create gift (integration)",
  () => {
...
describe.skipIf(!HAS_DB)(
  "Reconciliation approve — auto-proposed (`match_proposed`) rows stay reconcilable (integration)",
  () => {
...
describe.skipIf(!HAS_DB)(
  "Reconciliation approve — opportunity targets (integration)",
  () => {
...
describe.skipIf(!HAS_DB)(
  "Reconciliation graph — read-only proposer (integration)",
  () => {
    ...
    it("derives a determined donor node from a single confirmed donor match, and none when the QB row has no donor", async () => {
      ...
      const bareRes = await apiGet(
        `/api/reconciliation/cards/${bareStaged}/graph`,
      );
...
describe.skipIf(!HAS_DB)(
  "Reconciliation search — scoped + cross-filtering (integration)",
  () => {
...
describe.skipIf(!HAS_DB)(
  "Reconciliation cards — queue visibility (integration)",
  () => {
    it("default queue shows pending and hides match_confirmed; queue=done is the inverse", async () => {
      ...
      const liveIds = (
        live.json.data as Array<{ stagedPaymentId: string }>
      ).map((c) => c.stagedPaymentId);
      ...
      const doneIds = (
        done.json.data as Array<{ stagedPaymentId: string }>
      ).map((c) => c.stagedPaymentId);
