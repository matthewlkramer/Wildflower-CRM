 ...
 * panel. Computed from retained messages and the caller's unmatched headers:
 * takes every distinct
 ...
 *
 * `minMessages` defaults to 2. Historical skip headers have no thread id,
 * so we count distinct messages and report whether thread counts are complete.
 * `minThreads` remains an optional filter over known threads only.
 */
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const minMessages = q.minMessages;
    ...
    const rows = await db.execute<{
      ...
      sent_message_count: number | string;
      thread_count_complete: boolean;
      first_seen_at: Date;
      last_seen_at: Date;
      ...
      WITH sent_messages AS (
        SELECT
          ...
          sent_at,
          subject,
          COALESCE(to_emails, '{}'::text[]) || COALESCE(cc_emails, '{}'::text[]) AS recipients
        FROM email_messages
        ...
        UNION ALL
        SELECT s.mailbox_user_id, s.gmail_message_id, NULL::text,
          s.sent_at, s.subject, s.to_addrs || s.cc_addrs
        FROM email_sync_skip s
        JOIN users owner ON owner.id = s.mailbox_user_id
        LEFT JOIN google_oauth_tokens g ON g.user_id = owner.id
        -- Unmatched headers have no sharing consent. Even admins see only
        -- their own skipped mail, never another mailbox's private headers.
        WHERE s.mailbox_user_id = ${mailboxUserId}
          AND LOWER(COALESCE(NULLIF(g.google_email, ''), owner.email)) = ANY(s.from_addrs)
          AND s.sent_at >= ${cutoffSql}
          AND COALESCE(s.subject, '') !~* '^(Invitation:|Updated invitation:|Canceled event:|Accepted:|Declined:|Tentative:)'
          AND NOT EXISTS (
            SELECT 1 FROM email_messages retained
            WHERE retained.mailbox_user_id = s.mailbox_user_id
              AND retained.gmail_message_id = s.gmail_message_id
          )
      ), recents AS (
        SELECT *, LOWER(BTRIM(unnest(recipients))) AS addr FROM sent_messages
      ),
      candidate AS (
        SELECT
          mailbox_user_id,
          ...
          COUNT(DISTINCT gmail_message_id) AS sent_message_count,
          BOOL_AND(gmail_thread_id IS NOT NULL) AS thread_count_complete,
        ...
        WHERE addr <> ''
          AND addr LIKE '%@%'
          AND split_part(addr, '@', 2) <> ALL(ARRAY[${sql.join(
            internalDomains.map((d) => sql`${d}`),
            sql`, `,
          )}]::text[])
      ...
      WHERE c.sent_message_count >= ${minMessages}
        AND ${minThreads == null ? sql`TRUE` : sql`c.thread_count >= ${minThreads}`}
        AND c.email_address <> LOWER(u.email)
        AND NOT EXISTS (
          SELECT 1 FROM google_oauth_tokens self
          WHERE self.user_id = c.mailbox_user_id AND LOWER(self.google_email) = c.email_address
        )
        AND NOT EXISTS (
      ...
      ORDER BY c.sent_message_count DESC, c.last_seen_at DESC
      LIMIT 100
    `);
    ...
    const data = rows.rows.map((r) => {
      ...
      return {
        ...
        sentMessageCount: Number(r.sent_message_count),
        threadCountComplete: r.thread_count_complete,
        firstSeenAt:
      ...
      const result = await db.transaction(async (tx) => {
        ...
        if (body.proposalId) {
          const [proposal] = await tx
            ...
            .where(
              and(
                eq(emailProposals.id, body.proposalId),
          ...
          if (
            !proposal ||
            (proposal.mailboxUserId !== user.id && user.role !== "admin")
          ) {
            throw Object.assign(
              new Error("Proposal not found or is no longer pending."),
              { statusCode: 404 },
            );
          }
          if (
            proposal.kind !== "bounce_invalid" &&
            proposal.kind !== "bounce_soft"
          ) {
            throw Object.assign(
              new Error("Only bounce proposals can use the identity workflow."),
              { statusCode: 400 },
            );
          }
          if (
            body.invalidateObservedEmail &&
            proposal.kind !== "bounce_invalid"
          ) {
            throw Object.assign(
              new Error("Soft bounces cannot permanently invalidate an email."),
              { statusCode: 400 },
            );
          }
        } else if (body.invalidateObservedEmail) {
          throw Object.assign(
            new Error("Email invalidation requires a hard-bounce proposal."),
            { statusCode: 400 },
          );
        }
        ...
        if (!personId) {
          ...
          await tx.insert(people).values({
            ...
            fullName:
              ...
              [create.firstName, create.lastName].filter(Boolean).join(" ") ||
              null,
          });
        } else {
          const [person] = await tx
            ...
            .limit(1);
          if (!person)
            throw Object.assign(new Error("Person not found."), {
              statusCode: 404,
            });
        }
        ...
        const [existing] = await tx
          ...
          .from(emails)
          .where(sql`lower(${emails.email}) = lower(${emailAddress})`)
          .limit(1);
        let emailId: string;
        if (existing) {
          if (existing.personId !== personId) {
            throw Object.assign(
              new Error("That email is already attached to another record."),
              { statusCode: 409 },
            );
          }
          emailId = existing.id;
