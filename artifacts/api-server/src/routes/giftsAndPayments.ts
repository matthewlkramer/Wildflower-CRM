...
router.post(
  "/gifts-and-payments",
  asyncHandler(async (req, res) => {
    const body = parseOrBadRequest(CreateGiftOrPaymentBody, req.body, res);
    if (!body) return;
    const [row] = await db.insert(giftsAndPayments).values({ id: newId(), ...body } as any).returning();
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [row] = await db
      ...
      .where(eq(giftsAndPayments.id, paramId(req)))
      .returning();
    if (!row) return notFound(res, "gift");
    res.json(row);
  }),
