...
function serializeFeedback(
  ...
  const resolver = row.resolvedByUserId ? userMap.get(row.resolvedByUserId) : undefined;
  return {
    ...row,
    reporter: {
...
router.get("/admin/feedback", asyncHandler(async (req, res) => {
  ...
  if (category !== "all") filters.push(eq(appFeedback.category, category));
  if (query.search) {
    ...
    filters.push(sql`(
      ...
      OR EXISTS (
        ...
        WHERE feedback_user.id = ${appFeedback.createdByUserId}
          AND (feedback_user.email ILIKE ${term} ESCAPE '\\'
  ...
  const [rows, totalRows] = await Promise.all([
    db.select().from(appFeedback).where(where).orderBy(desc(appFeedback.createdAt), desc(appFeedback.id)).limit(limit).offset(offset),
    db.select({ value: count() }).from(appFeedback).where(where),
  ]);
  const userMap = await usersById(rows.flatMap((row) => [row.createdByUserId, row.resolvedByUserId]));
  res.json({
    data: rows.map((row) => serializeFeedback(row, userMap)),
    pagination: { page, limit, total: Number(totalRows[0]?.value ?? 0) },
  });
}));

router.patch("/admin/feedback/:id", asyncHandler(async (req, res) => {
  if (!requireAdmin(req, res)) return;
  const actor = getAppUser(req);
  if (!actor?.id) {
    res.status(401).json({ error: "unauthorized", message: "Sign in required." });
  ...
  const current = await db.query.appFeedback.findFirst({ where: eq(appFeedback.id, id) });
  if (!current) {
