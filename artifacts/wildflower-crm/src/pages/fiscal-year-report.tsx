...
export default function FiscalYearReport() {
  ...
  useEffect(() => {
    if (isAlias) {
      ...
      navigate(
        `/fiscal-year-report/${currentFiscalYearSlug()}?${next.toString()}`,
        { replace: true },
      );
    }
  ...
  const reportQ = useGetFiscalYearReport(fyId, reportParams, {
    query: {
      ...
      retry: (failureCount, error) => {
        const status = (error as { status?: number } | null | undefined)
          ?.status;
  ...
  const receivedRows = useMemo(
    () => rows.filter((r) => r.bucket === "received"),
    [rows],
  );
  const committedRows = useMemo(
    () => rows.filter((r) => r.bucket === "committed"),
    [rows],
  );
  ...
  const setCategory = (c: FundraisingCategory) => {
    ...
    const next = new URLSearchParams(search);
    next.set("category", c);
    navigate(`/fiscal-year-report/${fyId}?${next.toString()}`, {
      replace: true,
    });
  };
  ...
  // The API owns all rollups. Keep the server's weighted projection and goal
  // gap intact rather than reconstructing either from row values in the UI.
  const received = totals?.received ?? null;
  const committedWeighted = totals?.committedWeighted ?? null;
  const openWeighted = totals?.openWeighted ?? null;
  const projection = totals?.weightedProjection ?? null;
  const goalNum =
    totals?.goal != null && Number(totals.goal) > 0
      ? Number(totals.goal)
      : null;
  const hasGoal = goalNum != null;
  const projectionNum = projection == null ? null : Number(projection);
  const overGoal = hasGoal && projectionNum != null && projectionNum > goalNum;
  const coverage =
    hasGoal && projectionNum != null ? projectionNum / goalNum! : null;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          ...
          received goal credit, unpaid commitments, and open asks. The
          projection is probability-weighted; face-value amounts are shown
          separately. Totals reconcile to the dashboard bar. Scoped to{" "}
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div
          ...
          data-testid="report-category-toggle"
        >
              ...
              <SelectContent>
                {visibleFyOptions.map((f) => (
                  <SelectItem
                    key={f.id}
                    value={f.id}
                    data-testid={`select-fy-option-${f.id}`}
                  >
                    {f.label}
                  </SelectItem>
                ))}
      ...
      {showError ? (
        ...
        <>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-baseline justify-between gap-4">
                ...
                <div
                  ...
                  data-testid="report-summary-total"
                >
                  {reportQ.isLoading ? (
                    ...
                    <span>
                      ...
                      {overGoal ? (
                        <span className="ml-1 text-emerald-600">
                          (+{formatCurrency(projectionNum! - goalNum!)} over)
                        </span>
                      ) : null}
                    </span>
                  ) : (
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                <SummaryTile
                  ...
                  value={fmt(received)}
                  testId="tile-received"
                />
                <SummaryTile
                  label="Face-value unpaid commitments"
                  value={fmt(totals?.committed)}
                  testId="tile-committed-face"
                />
                <SummaryTile
                  label="Probability-weighted unpaid commitments"
                  value={fmt(committedWeighted)}
                  testId="tile-committed"
                />
                <SummaryTile
                  label="Face-value open asks"
                  value={fmt(totals?.openAsk)}
                  testId="tile-open-face"
                />
                <SummaryTile
                  label="Weighted open asks"
                  value={fmt(openWeighted)}
                  testId="tile-open"
                />
                <SummaryTile
                  label="Goal gap"
                  value={fmt(totals?.goalGap)}
                  testId="tile-goal-gap"
                />
                <SummaryTile
                  label="Goal"
                  value={fmt(totals?.goal ?? null)}
                  testId="tile-goal"
                />
              </div>
            </CardContent>
          ...
          <ReceivedTable
            rows={receivedRows}
            total={totals?.received ?? null}
            isLoading={reportQ.isLoading}
          />
          <CommittedTable
            rows={committedRows}
            totalAmount={totals?.committed ?? null}
            totalWeighted={totals?.committedWeighted ?? null}
            isLoading={reportQ.isLoading}
          />
          <OpenTable
            rows={openRows}
            totalAsk={totals?.openAsk ?? null}
            totalWeighted={totals?.openWeighted ?? null}
            isLoading={reportQ.isLoading}
          />
        </>
      )}
...
function ReceivedTable({
  ...
  const ts = useTableState("fy-report-received", {
    key: "amount",
    dir: "desc",
  });
  ...
  return (
    <div className="space-y-2" data-testid="report-bucket-received">
      <BucketHeading
        title="Received goal credit"
        subtitle="Cash in — gift allocations booked to this fiscal year."
      />
      <div className="rounded-md border bg-card overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <SortableTH colKey="dateReceived" {...ts}>
                Date received
              </SortableTH>
              <SortableTH colKey="donor" {...ts}>
                Donor
              </SortableTH>
              <SortableTH colKey="usage" {...ts}>
                Usage
              </SortableTH>
              <SortableTH colKey="amount" align="right" {...ts}>
                Amount
              </SortableTH>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                ...
                >
                  No gifts received in this fiscal year + track.
                </TableCell>
              </TableRow>
            ) : (
              <>
                    ...
                    <TableCell>
                      <DonorCell
                        ...
                        individualGiverPersonPriority={
                          r.individualGiverPersonPriority
                        }
                      />
                    </TableCell>
                    <TableCell className="text-muted-foreground">
                      {r.displayUsage
                        ? abbreviateUsStates(r.displayUsage)
                        : "—"}
                    </TableCell>
                ...
                <TableRow className="bg-muted/30 font-medium">
                  <TableCell colSpan={3}>Total received goal credit</TableCell>
                  <TableCell
                    className="text-right tabular-nums"
                    data-testid="row-received-total"
                  >
                    {fmt(total)}
                  </TableCell>
                </TableRow>
...
function CommittedTable({
  ...
  totalWeighted: string | null;
  isLoading: boolean;
}) {
  const ts = useTableState("fy-report-committed", {
    key: "amount",
    dir: "desc",
  });
  const sortedRows = useMemo(
    () =>
      sortRows(
        rows,
        {
          ...
          opportunity: (r) =>
            (r.opportunityName ?? r.opportunityId ?? "").toLowerCase(),
          pledged: (r) =>
            r.pledgedAmount != null ? Number(r.pledgedAmount) : null,
          paid: (r) => (r.paidAmount != null ? Number(r.paidAmount) : null),
          winProb: (r) =>
            r.winProbability != null ? Number(r.winProbability) : null,
          amount: (r) => (r.amount != null ? Number(r.amount) : null),
          weighted: (r) =>
            r.weightedAmount != null ? Number(r.weightedAmount) : null,
        },
        ts.sort,
      ),
    [rows, ts.sort],
  ...
  return (
    <div className="space-y-2" data-testid="report-bucket-committed">
      <BucketHeading
        ...
        subtitle="Face value and probability-weighted remainder of written pledges (pledged − paid this FY)."
      />
      <div className="rounded-md border bg-card overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <SortableTH colKey="opportunity" {...ts}>
                Opportunity
              </SortableTH>
              <SortableTH colKey="pledged" align="right" {...ts}>
                Pledged
              </SortableTH>
              <SortableTH colKey="paid" align="right" {...ts}>
                Paid
              </SortableTH>
              <SortableTH colKey="winProb" align="right" {...ts}>
                Win prob
              </SortableTH>
              <SortableTH colKey="amount" align="right" {...ts}>
                Face-value unpaid commitment
              </SortableTH>
              <SortableTH colKey="weighted" align="right" {...ts}>
                Probability-weighted unpaid commitment
              </SortableTH>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell
                  ...
                  className="text-center h-24 text-muted-foreground"
                >
                  No committed pledges in this fiscal year + track.
                </TableCell>
              </TableRow>
            ) : (
              <>
                {sortedRows.map((r) => (
                  <TableRow key={r.rowId} data-testid={`row-${r.rowId}`}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {pct(r.winProbability)}
                    </TableCell>
                    <TableCell className="text-right tabular-nums font-medium">
                      {fmt(r.amount)}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {fmt(r.weightedAmount)}
                    </TableCell>
                  </TableRow>
                ))}
                <TableRow className="bg-muted/30 font-medium">
                  <TableCell colSpan={5}>Total unpaid commitments</TableCell>
                  <TableCell
                    className="text-right tabular-nums"
                    data-testid="row-committed-total"
                  >
                    {fmt(totalAmount)}
                  </TableCell>
                  <TableCell
                    className="text-right tabular-nums"
                    data-testid="row-committed-total-weighted"
                  >
                    {fmt(totalWeighted)}
                  </TableCell>
                </TableRow>
...
function OpenTable({
  ...
  return (
    <div className="space-y-2" data-testid="report-bucket-open">
      <BucketHeading
        title="Open asks"
        subtitle="Face-value open asks and probability-weighted open asks."
      />
      <div className="rounded-md border bg-card overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <SortableTH colKey="projectedClose" {...ts}>
                Projected close
              </SortableTH>
              <SortableTH colKey="donor" {...ts}>
                Donor
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell
                  ...
                  className="text-center h-24 text-muted-foreground"
                >
                  No open opportunities in this fiscal year + track.
                </TableCell>
              </TableRow>
            ) : (
              <>
                ...
                <TableRow className="bg-muted/30 font-medium">
                  <TableCell colSpan={5}>Total open asks</TableCell>
                  <TableCell
                    className="text-right tabular-nums"
                    data-testid="row-open-total-ask"
                  >
                    {fmt(totalAsk)}
                  </TableCell>
                  <TableCell
                    className="text-right tabular-nums"
                    data-testid="row-open-total-weighted"
                  >
                    {fmt(totalWeighted)}
                  </TableCell>
                </TableRow>
