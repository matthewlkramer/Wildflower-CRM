...
// ── Google connection ────────────────────────────────────────────────────────
// Per-user OAuth grant for Gmail + Calendar sync. The token row lives
// in `google_oauth_tokens`; the actual sync workers (T003 / T004)
// consume it.
...
function GoogleConnectSection() {
  ...
  const statusQ = useGetGoogleOauthStatus({
    query: { queryKey: getGetGoogleOauthStatusQueryKey(), refetchOnWindowFocus: true },
  });
  ...
  // This runs once on mount; we strip the query param so a refresh
  ...
  useMemo(() => {
    ...
    if (result === "connected") {
      toast({ title: "Google connected", description: "Gmail & Calendar sync will start on the next cycle." });
    } else if (result === "denied") {
      toast({ title: "Connection cancelled", description: "Google sign-in was cancelled.", variant: "destructive" });
    }
  ...
  const handleConnect = () => {
    // Browser navigation — the start route 302s to Google. We can't
    // do this via fetch because Google's consent screen has to be the
    // top-level navigation.
    window.location.href = `/api/google-oauth/start?returnTo=${encodeURIComponent("/admin")}`;
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Link your Google account so your emails and meetings with people in the
          CRM automatically show up on their detail pages. Each staff member
          connects their own mailbox; you can disconnect any time.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {statusQ.isLoading ? (
          ...
          <div className="rounded-md border border-yellow-300 bg-yellow-50 p-3 text-sm text-yellow-900">
            Google OAuth isn't configured on the server yet. An admin needs to
          ...
          <div className="space-y-3">
            <div className="text-sm">
              ...
              {status.lastError ? (
                <div className="text-red-700 mt-1">Last sync error: {status.lastError}</div>
              ) : null}
            </div>
            <div className="flex gap-2">
              ...
              <Button variant="ghost" onClick={handleConnect} data-testid="google-reconnect">
                Reconnect
              </Button>
            </div>
          </div>
