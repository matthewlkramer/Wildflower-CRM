...
export function groupTripTravelBookings(
  bookings: TripPlanDetail["travelBookings"],
) {
  return {
    flights: bookings.filter((booking) => booking.kind === "flight"),
    hotels: bookings.filter((booking) => booking.kind === "hotel"),
  };
}

function TravelBookingSummary({ trip }: { trip: TripPlanDetail }) {
  const { flights, hotels } = groupTripTravelBookings(trip.travelBookings);
  const BookingRows = ({
    bookings,
  }: {
    bookings: TripPlanDetail["travelBookings"];
    ...
    <div className="space-y-2">
      {bookings.map((booking) => (
        <div
          key={`${booking.source}-${booking.sourceId}`}
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              !booking.title
                .toLowerCase()
                .includes(booking.provider.toLowerCase()) ? (
            ...
            <div className="flex items-center gap-1">
              <Badge variant="secondary">
                {booking.source === "calendar" ? "Calendar" : "Gmail"}
              </Badge>
              {booking.sourceUrl ? (
  ...
  return (
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <div className="rounded-md bg-muted p-3 text-sm">
        ...
        {flights.length ? (
          <BookingRows bookings={flights} />
        ) : (
          <Badge variant="outline">No flights</Badge>
        )}
      </div>
      <div className="rounded-md bg-muted p-3 text-sm">
        <div className="mb-2 flex items-center gap-2">
          <Hotel className="h-4 w-4" />
          <p className="text-xs font-medium text-muted-foreground">Hotel</p>
        </div>
        {hotels.length ? (
          <BookingRows bookings={hotels} />
        ) : (
          <Badge variant="outline">No hotel booked</Badge>
        )}
      </div>
    </div>
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            <Label htmlFor={`trip-visit-next-step-${visit.id}`}>
              Next step
            </Label>
            <Textarea
...
function Schedule({ trip }: { trip: TripPlanDetail }) {
  ...
  const calendarBookings = useMemo(
    () =>
      new Map(
        trip.travelBookings
          .filter((booking) => booking.source === "calendar")
          .map((booking) => [booking.sourceId, booking]),
      ),
    [trip.travelBookings],
  );
  const gmailScheduleBookings = useMemo(
    () =>
      trip.travelBookings
        .filter((booking) => booking.source === "gmail" && !!booking.startAt)
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        {gmailScheduleBookings.length ? (
          <div>
            <h3 className="mb-2 text-sm font-semibold">
              Travel bookings from Gmail
            </h3>
            <div className="space-y-2 border-l-2 border-primary/30 pl-4">
              {gmailScheduleBookings.map((booking) => (
                <div
                  key={`gmail-${booking.sourceId}`}
                ...
                >
                  <p className="flex flex-wrap items-center gap-2 font-medium">
                    ...
                    {booking.title}
                    <Badge variant="secondary">
                      {booking.kind === "flight" ? "Flight" : "Hotel"}
                    </Badge>
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {bookingDateLabel(booking.startAt, booking.endAt)}
            ...
            <div key={day}>
              ...
              <div className="space-y-2 border-l-2 border-primary/30 pl-4">
                {events.map(({ event, birthday, manuallyHidden, hidden }) => {
                  const travelBooking = calendarBookings.get(event.id);
                  return (
                    <div
                      key={event.id}
                      className={`rounded-md border bg-background p-3 ${
    ...
    <div className="space-y-5">
      <Card>
        <CardContent className="p-5">
          ...
          <TravelBookingSummary trip={trip} />
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
