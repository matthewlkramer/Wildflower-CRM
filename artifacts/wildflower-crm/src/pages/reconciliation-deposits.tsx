    ...
    <div className="space-y-4">
      ...
      <AlertDialog open={accountingDispositionFor != null} onOpenChange={(open) => { if (!open && !busy) setAccountingDispositionFor(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {accountingDispositionFor?.disposition === "accepted_historical" ? "Accept historical accounting?" : "Mark accounting corrected?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              This records a human review decision on the accounting check only. It does not change money or gift relationships.
            </AlertDialogDescription>
          </AlertDialogHeader>
          {accountingDispositionFor?.disposition === "accepted_historical" ? (
            <Textarea
              value={accountingDispositionNote}
              onChange={(event) => setAccountingDispositionNote(event.target.value)}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={busy}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              disabled={busy || (accountingDispositionFor?.disposition === "accepted_historical" && !accountingDispositionNote.trim())}
