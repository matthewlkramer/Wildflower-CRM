...
import { giftPaymentMethodEnum, loanOrGrantEnum } from "./_enums";
...
export const giftsAndPayments = pgTable(
  "gifts_and_payments",
  {
    ...
    // linked to any opportunity, not only a committed pledge). Gift type is
    // derived from the linked record's lifecycle: a finalized pledge produces a
    // pledge payment/reimbursement; a non-pledge opportunity produces a standard
    // stand-alone gift outcome while preserving its fundraising history.
    ...
    thankYouLetterUploadedAt: timestamp("thank_you_letter_uploaded_at", {
      mode: "string",
    }),
  ...
  (t) => [
    ...
    index("gifts_and_payments_individual_giver_person_id_idx").on(
    ...
    index("gifts_and_payments_gift_being_matched_id_idx").on(
      t.giftBeingMatchedId,
    ),
    index("gifts_and_payments_primary_contact_person_id_idx").on(
      t.primaryContactPersonId,
    ),
    index("gifts_and_payments_payment_intermediary_id_idx").on(
    ...
    index("gifts_and_payments_thank_you_email_msg_idx").on(
      t.thankYouEmailMessageId,
    ),
