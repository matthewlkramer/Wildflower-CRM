...
// Normalize legacy grant-year codes (FY24 → fy2024) into fiscal_years.id slugs.
// "Future" from Airtable is dropped (the sentinel row was removed); callers
// should treat it as an unset grant year.
const normalizeFy = (gy) => {
  if (gy == null) return null;
  if (/^FY\d{2}$/.test(gy)) return "fy20" + gy.slice(2);
  if (gy === "Future") return null;
...
// Legacy commitment labels ("Conditional commitment", "Written commitment",
// "Cash in") no longer map to the deprecated stage enum values. They import as
// the new model: the cultivation stage is set to its pre-win funnel position
// (verbal_confirmation), `written_pledge` is latched true, and `conditional` is
// filled for conditional commitments. The derive backfill then advances won
// rows to `complete` and re-derives status/win_probability.
const oppStageMap = {
  "Cold Lead - 0%": "cold_lead",
  "Warm Lead - 5%": "warm_lead",
  "In conversation - 20%": "in_conversation",
  "Convince - 40%": "convince",
  "Conditional commitment - 50%": "verbal_confirmation",
  "Probable Renewal - 75%": "probable_renewal",
  "Verbal commitment - 90%": "verbal_confirmation",
  "Written commitment - 100%": "verbal_confirmation",
  "Cash in - 100%": "verbal_confirmation",
};
// Legacy stage labels that latch the sticky `written_pledge` flag on import.
const LEGACY_COMMITMENT_STAGE_LABELS = new Set([
  "Conditional commitment - 50%",
  "Written commitment - 100%",
  "Cash in - 100%",
]);
// Legacy stage labels that imply a conditional commitment (used to fill
// `conditional` when the dedicated Airtable column is absent/unconditional).
const LEGACY_CONDITIONAL_STAGE_LABELS = new Set(["Conditional commitment - 50%"]);
...
const ENTITY_DEFS = [
  ...
  // "Sunlight - equity" was merged into "Sunlight - grants" per business
  // decision (the equity entity was never funded in practice); the equity
  // aliases route here so any stray Airtable references reroute cleanly.
  { id: "sunlight_grants", name: "Sunlight - grants",
    aliases: ["Sunlight - grants", "Sunlight Grants", "Sunlight - equity", "Sunlight - Equity"] },
...
async function run() {
  ...
  // Synthetic top-level region representing the United States as a whole.
  // Not present in the Airtable source — it models funders that were
  // previously flagged national_priorities=true (that boolean column was
  // removed; national scope is now expressed as this region in region_ids).
  if (!regionRows.some((r) => r.id === "united-states")) {
    regionRows.push({
      id: "united-states",
      name: "United States",
      display_path: "United States",
  ...
  await batchInsert(
    ...
    [
      ...
      "capacity_rating","priority_areas_notes","active_status","other_names",
  ...
  // 10. phone_numbers
  // phone_numbers carries the same 5-owner XOR pattern as emails/addresses,
  // but Airtable only ships person-linked phones (verified via API audit:
  // 1199 person + 4 truly-orphan, 0 funder/org/household/PI). Drop the 4
  // orphan rows rather than violate the num_nonnulls=1 CHECK; the owner
  // columns exist for future records added via the API.
  const phones = load("phone_numbers");
  const phoneDrops = { no_phone: 0, no_owner: 0 };
  await batchInsert(
    "phone_numbers",
    ["id","phone_number","type","person_id","funder_id","organization_id",
     "payment_intermediary_id","household_id","validity","is_preferred"],
    phones.flatMap((r) => {
      const f = r.fields;
      if (!f.phone_number) { phoneDrops.no_phone++; return []; }
      const pid = first(f.person);
      const person_id = pid && personIds.has(pid) ? pid : null;
      // Airtable's phones table has no funder/org/household/pi link fields
      // today, so person is the only possible owner. If/when those fields
      // are added upstream, extend this mapping accordingly.
      if (!person_id) { phoneDrops.no_owner++; return []; }
      return [{
        id: r.id,
  ...
  if (phoneDrops.no_owner || phoneDrops.no_phone) {
    console.log(`  phone_numbers drops:`, phoneDrops);
  }
  ...
  await batchInsert(
    ...
    ["id","name","funder_id","ask_amount","awarded_amount","type","conditional",
     "written_pledge","individual_giver_person_id","status","owner_user_id",
     "projected_close_date","actual_completion_date","win_probability","stage",
     "payment_details","usage_notes","copper_pledge_id","primary_contact_person_id",
    ...
    opps.map((r) => {
      ...
      // Legacy commitment stages import as the new model: latch written_pledge
      // and, for conditional commitments, fill `conditional` when the dedicated
      // column is absent/unconditional. The derive backfill re-derives final
      // stage (`complete` on win) and status/win_probability afterward.
      const isLegacyCommitment = LEGACY_COMMITMENT_STAGE_LABELS.has(f.stage);
      let conditional = oppConditionalMap[f.conditional] ?? null;
      if (
        LEGACY_CONDITIONAL_STAGE_LABELS.has(f.stage) &&
        (conditional === null || conditional === "unconditional")
      ) {
      ...
      return {
        ...
        conditional,
        written_pledge: isLegacyCommitment,
  ...
  await batchInsert(
    ...
    ["id","legacy_gift_id","name","details","date_received","payment_method",
     "amount","funder_id","individual_giver_person_id","type","opportunity_id",
    ...
    gifts.map((r) => {
      ...
      return {
        ...
        opportunity_id: pop && oppIds.has(pop) ? pop : null,
  ...
  // Funders flagged national_priorities=true in Airtable are modeled as having
  // the United States as a whole in their region_ids (the national_priorities
  // boolean column was removed from the schema). This runs after setRegionIds
  // overwrites region_ids from Airtable's regional_priorities, and appends the
  // synthetic 'united-states' slug idempotently so re-imports stay correct.
  console.log("== funders: national_priorities -> united-states region ==");
  for (const r of funders) {
    if (r.fields.national_priorities !== true) continue;
    await pool.query(
      `UPDATE funders
         SET region_ids = COALESCE(region_ids, '{}'::text[]) || ARRAY['united-states']
       WHERE id = $1
         AND NOT (COALESCE(region_ids, '{}'::text[]) @> ARRAY['united-states'])`,
      [r.id],
    );
  }
