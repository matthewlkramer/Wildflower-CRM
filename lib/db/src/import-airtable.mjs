...
async function run() {
  ...
  await batchInsert(
    ...
    ["id","airtable_id","name","funder_id","ask_amount","awarded_amount","type","conditional",
     ...
     "intended_usage","fundable_project_id","usage_notes","pledge_id","primary_contact_person_id",
  ...
  // 12b. opportunity_entities (junction from old opps.entity array)
  const oppEntJ = [];
  for (const r of opps) {
    if (!oppIds.has(r.id)) continue;
    for (const raw of arr(r.fields.entity)) {
      const eid = entityAliasToId.get(raw);
      if (eid) oppEntJ.push({ opportunity_id: r.id, entity_id: eid });
    }
  }
