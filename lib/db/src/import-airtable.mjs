...
async function run() {
  ...
  await batchInsert(
    "organizations",
    ["id","airtable_id","name","type","email_domain","owner_user_id","website"],
    orgs.map((r) => {
      const f = r.fields;
      return {
        id: r.id, airtable_id: r.id,
  ...
  async function setRegionIds(table, sourceRows, airtableField, idAllowSet) {
    for (const r of sourceRows) {
      ...
      const ids = arr(r.fields[airtableField]).filter((rid) => regionIds.has(rid));
      if (!ids.length) continue;
      await pool.query(`UPDATE ${table} SET region_ids = $1 WHERE id = $2`, [ids, r.id]);
    }
  }
