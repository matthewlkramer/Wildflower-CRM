...
export const ListReconciliationCardsResponse = zod.object({
  ...
  "stripeChargeDonorName": zod.string().nullish().describe('Payer\/customer name on the single Stripe charge backing this money — the real donor, not the \'Stripe\' processor name that appears as the QB payer. Null for multi-charge payouts or non-Stripe money.'),
  "stripeGrossAmount": zod.string().nullish().describe('Gross amount (major units) of the single Stripe charge backing this money; null for multi-charge payouts or non-Stripe money.'),
  "stripeNetAmount": zod.string().nullish().describe('Net amount deposited (gross − processor fee) for the single backing Stripe charge; null for multi-charge payouts or non-Stripe money.'),
  "stripeFeeAmount": zod.string().nullish().describe('Processor fee on the single backing Stripe charge (gross − net); null for multi-charge payouts or non-Stripe money.'),
  ...
  "syncGap": zod.boolean().describe('Plain human-set \'sync gap\' flag — the gift exists in the CRM but was missing from the QuickBooks export; orthogonal to reconcile status. Set via \/staged-payments\/{id}\/set-sync-gap.'),
