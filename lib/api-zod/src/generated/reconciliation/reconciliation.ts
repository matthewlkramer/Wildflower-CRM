...
export const SetBankDepositExclusionBody = zod.object({
  "reason": zod.enum(['membership', 'lease_guaranty', 'loan_repayment', 'loan_proceeds', 'note_payable', 'earned_income', 'interest', 'other_revenue', 'intercompany_transfer', 'tax_refund', 'insurance', 'expense_refund', 'expensify', 'returned_wire', 'miscoded_withdrawal', 'non_wf', 'other']),
 ...
 */
export const listWorkbenchDepositsQueryLimitDefault = 50;
export const listWorkbenchDepositsQueryLimitMax = 10000;
...
export const ListWorkbenchDepositsResponse = zod.object({
  ...
  "sourceInvoiceId": zod.string().nullish(),
  "invoiceDocNumber": zod.string().nullish(),
  "invoicePurpose": zod.string().nullish().describe('Purpose copied read-only from the linked QuickBooks invoice.'),
  ...
  "recordComplete": zod.boolean().optional().describe('True when the gift satisfies the canonical CRM record-completeness predicate. Exposed per-gift so the UI can highlight incomplete records without reproducing the rule.'),
