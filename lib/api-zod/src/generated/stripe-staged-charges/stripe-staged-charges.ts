...
export const ListStripeStagedChargesQueryParams = zod.object({
  "queue": zod.enum(['needs_review', 'auto_matched', 'excluded', 'done', 'refund_review']).optional().describe('Which queue to list (default needs_review).'),
...
export const ListStripeStagedChargesResponse = zod.object({
  ...
  "queue": zod.enum(['needs_review', 'auto_matched', 'excluded', 'done', 'refund_review']).optional().describe('Derived queue bucket. needs_review: pending rows. auto_matched: match_proposed (a system-applied gift link awaiting review). done: match_confirmed evidence rows tied to a CRM gift — hidden from the active work queues but filterable on demand. refund_review: a Stripe-only cross-cutting filter (independent of status) for charges with an open refund\/chargeback proposal awaiting confirm\/dismiss — never emitted as a row\'s derived bucket.'),
