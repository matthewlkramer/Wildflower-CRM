...
export const ListOpportunitiesAndPledgesQueryParams = zod.object({
  ...
  "projectedCloseDatePresence": zod.enum(['has', 'blank']).optional().describe('Presence filter on projected close date (`has` = set, `blank` = null).'),
  "applicationDeadlinePresence": zod.enum(['has', 'blank']).optional().describe('Presence filter on application deadline (`has` = set, `blank` = null).'),
  "winProbabilityPresence": zod.enum(['has', 'blank']).optional().describe('Presence filter on win probability (`has` = set, `blank` = null).'),
  ...
  "fundableProjectId": zod.array(zod.coerce.string()).optional().describe('Filter to opportunities\/pledges that have at least one pledge_allocation\nwith fundable_project_id in the given set. Comma-separated form supported.\n'),
...
export const ListOpportunitiesAndPledgesResponse = zod.object({
  ...
  "conditionsMetRollup": zod.enum(['no', 'partial', 'yes']).describe('Tri-state record of whether a grant\'s conditions have been met:\n\'no\' (none met), \'partial\' (some met), or \'yes\' (fully met).\nReplaces the prior boolean flag (false→\'no\', true→\'yes\').\n').optional().describe('Derived from the opportunity\'s pledge allocations: yes when every conditional allocation is met, partial when at least one is met or partly met, otherwise no.'),
  "restricted": zod.boolean().nullish().describe('Derived from the active reviewed term set, allocation donor-restriction axes, and active force-restricted entity policy. Null only while restriction status is not yet determinable.'),
  "restrictionBasis": zod.array(zod.string()).optional().describe('Human-readable reasons supporting the derived restricted result.'),
  ...
  "reportingRequired": zod.boolean().nullable().describe('CRM-native statement that this opportunity or award requires one or more donor reports. Null means the historical record has not been reviewed and blocks linked gifts from accounting export. When true, a reporting-deadline task is also required.'),
...
export const CreateOpportunityOrPledgeBody = zod.object({
  ...
  "commitmentPath": zod.enum(['gift', 'written_pledge', 'verbal_pledge']).optional().describe('The positive outcome the donor verbally confirmed; not itself an actual pledge or gift.'),
  "verbalCommitmentAt": zod.string().date().optional(),
  ...
  "reportingRequired": zod.boolean().describe('Explicit yes\/no decision about whether the opportunity or award requires donor reporting.'),
...
export const UpdateOpportunityOrPledgeBody = zod.object({
  ...
  "paymentDetails": zod.string().nullish(),
  "usageNotes": zod.string().nullish(),
  "reportingRequired": zod.boolean().optional().describe('Whether the opportunity or award requires donor reporting.'),
  "copperPledgeId": zod.string().nullish(),
  "grantLetterUrl": zod.string().nullish(),
  "grantLetterFilename": zod.string().nullish(),
  "grantLetterUploadedAt": zod.string().datetime({}).nullish(),
 ...
 */
export const ReduceOpportunityPlanProportionallyParams = zod.object({
  "id": zod.coerce.string()
})

export const ReduceOpportunityPlanProportionallyBody = zod.object({
...
export const ReduceOpportunityPlanProportionallyResponse = zod.object({
  ...
  "oldTarget": zod.string(),
  "newTarget": zod.string(),
  "allocationsAdjusted": zod.number(),
  "installmentsAdjusted": zod.number(),
  "postedPaymentsAdjusted": zod.literal(0).describe('Always zero: posted payment facts are never edited by this action.')
})
 ...
 */
export const ConvertPledgeToStandaloneGiftParams = zod.object({
  "id": zod.coerce.string()
})

export const ConvertPledgeToStandaloneGiftBody = zod.object({
  "reason": zod.string().nullish().describe('Optional explanation stored in the audit history.')
})

export const ConvertPledgeToStandaloneGiftResponse = zod.object({
  "giftId": zod.string(),
  "opportunityId": zod.string().describe('The rewritten originating opportunity, now completed as a gift outcome.')
})

/**
 * @summary Revert a mischaracterized pledge to a verbally committed gift opportunity awaiting payment.
 */
export const RevertPledgeToVerbalGiftParams = zod.object({
  "id": zod.coerce.string()
})

export const RevertPledgeToVerbalGiftBody = zod.object({
 ...
 */
export const RevertPledgeToOpportunityParams = zod.object({
  "id": zod.coerce.string()
})

export const RevertPledgeToOpportunityBody = zod.object({
  "stage": zod.enum(['cold_lead', 'warm_lead', 'in_conversation', 'convince', 'conditional_commitment', 'probable_renewal', 'verbal_confirmation', 'written_commitment', 'cash_in', 'complete']).describe('Cultivation funnel position, separate from commitment and actual outcome.\nActive stages end at verbal_confirmation. Pledge finalization and payment\ndo not overwrite the recorded stage. conditional_commitment,\nwritten_commitment, cash_in, and complete remain only for historical API\ncompatibility and are normalized to verbal_confirmation by migration 0224.\n').nullish().describe('Open funnel stage after correction; defaults to in_conversation.'),
  "projectedCloseDate": zod.string().date().nullish(),
  "reason": zod.string().nullish()
})
