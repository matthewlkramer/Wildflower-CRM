...
export const ListGiftsAndPaymentsQueryParams = zod.object({
  ...
  "restrictionLabels": zod.array(zod.enum(['unrestricted', 'purpose', 'time', 'both'])).optional().describe('Filter by restriction summary label (OR semantics). `unrestricted` = no donor-restricted axis; `purpose` = regional or usage donor-restricted but not time; `time` = time donor-restricted but not regional\/usage; `both` = time AND (regional or usage) donor-restricted.'),
...
export const ListGiftsAndPaymentsResponse = zod.object({
  ...
  "restrictionLabel": zod.string().nullish().describe('Derived restriction summary across allocations: Unrestricted \/ Purpose \/ Time \/ Both. Mirrors GiftAuditReconciliation.restrictionType (regional\/usage donor_restricted ⇒ Purpose; time donor_restricted ⇒ Time). Null when the gift has no allocations.'),
