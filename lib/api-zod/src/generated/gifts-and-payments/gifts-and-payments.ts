...
export const ListGiftsAndPaymentsResponse = zod.object({
  ...
  "processorFee": zod.string().nullish().describe('Processor fee withheld (e.g. Stripe per-charge fee). Donor is credited the GROSS `amount`; net = amount − processorFee. Null for gifts with no processor fee.'),
  "originalHumanCrmAmount": zod.string().nullish().describe('Snapshot of the human-entered amount before any processor reconciliation overwrote `amount`. Lets the UI show \'you entered $X, Stripe says $Y\'. Null only for a gift minted directly from a payment.'),
  "finalAmountSource": zod.enum(['human', 'stripe', 'quickbooks']).describe('Where a gift\'s final `amount` was last sourced from. human: hand-entered, never reconciled. stripe: stamped from a Stripe charge (gross). quickbooks: stamped from a QuickBooks staged row. XOR with the two final_amount pointer fields.').describe('Where `amount` was last sourced from. XOR with the two pointer fields below.'),
  "finalAmountStripeChargeId": zod.string().nullish().describe('The Stripe charge (gross) this gift\'s amount was stamped from, when finalAmountSource is stripe.'),
  "finalAmountQbStagedPaymentId": zod.string().nullish().describe('The QuickBooks staged row this gift\'s amount was stamped from, when finalAmountSource is quickbooks.'),
  ...
  "countsTowardGoal": zod.boolean().describe('When false, this real money is excluded from goal\/received analytics rollups (e.g. reimbursement grants that don\'t cover core expenses). Orthogonal to paymentExpected. Defaults true.'),
