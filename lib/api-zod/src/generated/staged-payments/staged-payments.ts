...
export const ListStagedPaymentsResponse = zod.object({
  ...
  "needsResearch": zod.boolean().describe('Plain human-set flag: a reviewer hasn\'t fully figured this incoming money out yet (unknown donor, ambiguous coding, unclear restriction, etc.). Never auto-derived; set via \/set-needs-research with no side effects on reconcile status or matching.'),
  "syncGap": zod.boolean().describe('Plain human-set flag: this QuickBooks money already exists in the CRM as a gift but was missing from the QuickBooks export (a sync gap). Never auto-derived; set via \/set-sync-gap with no side effects on reconcile status or matching.'),
...
export const SetStagedPaymentNeedsResearchBody = zod.object({
  "needsResearch": zod.boolean().describe('True to flag this row for further research, false to clear it.')
}).describe('Set or clear the plain human \'needs research\' flag on a staged payment. A pure annotation with no side effects on reconcile status or matching.')
 ...
 */
export const SetStagedPaymentSyncGapParams = zod.object({
  "id": zod.coerce.string()
})

export const SetStagedPaymentSyncGapBody = zod.object({
  "syncGap": zod.boolean().describe('True to flag this row as a sync gap, false to clear it.')
}).describe('Set or clear the plain human \'sync gap\' flag on a staged payment (the gift exists in the CRM but was missing from the QuickBooks export). A pure annotation with no side effects on reconcile status or matching.')

export const SetStagedPaymentSyncGapResponse = zod.object({
  "id": zod.string(),
  "realmId": zod.string(),
  "qbEntityType": zod.enum(['sales_receipt', 'payment', 'deposit']),
  "qbEntityId": zod.string(),
...
export const GroupReconcileStagedPaymentsBody = zod.object({
  ...
  "confirmAmountMismatch": zod.boolean().optional().describe('Must be true when the members\' combined total falls OUTSIDE the processor fee-band tolerance around the gift amount (e.g. stock\/securities gifts, where the cash from selling the shares differs from the gift\'s booked value). Lets the operator explicitly approve a deliberate amount mismatch; without it such a group is rejected 400 amount_mismatch_confirmation_required. The client prompts the operator to confirm before sending this.')
