...
2. **Deploy the CRM app first** (Replit "Publish" button) to get a stable `.replit.app` domain, then build against that domain:
   ```bash
   cd tools/magio-extension
   PLASMO_PUBLIC_API_URL=https://<your-deployed-domain>.replit.app pnpm build
   pnpm package
   # zip lands at build/chrome-mv3-prod.zip — copy to wildflower-tracking-extension.zip
   ```
   ...
   **Important:** The API URL is baked into the extension at build time. Always build against the production/deployed URL, not the dev preview URL, before submitting to the Web Store.

3. Upload `build/wildflower-tracking-extension.zip` to the Chrome Web Store developer dashboard for this item.
...
The current build in `tools/magio-extension/build/wildflower-tracking-extension.zip` is pointed at the **Replit dev preview domain** for testing only. Rebuild against the production domain before uploading to the Web Store.
